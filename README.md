...
ensuring marginal termination probabilities $P_T(x) = ∑_{τ→x} P_F(τ) ∝ R(x)$, with Z as the partition function. Detailed balance (DB) localizes this: F(s) P_F(s' | s) = F(s') P_B(s | s'), where F(s) = ∑{τ∋s} F(τ) aggregates flows through state s.
