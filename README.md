...
This setup aims to train GFlowNets to sample matchings H proportional to rewards R(H) (e.g., number of transplants), but it reveals fundamental fractures when applied to multi-round stochastic environments. Standard GFlowNets enforce equilibrium in deterministic DAGs: for a trajectory $τ = (s_0 → ... → s_n = x)$, trajectory balance (TB) requires
